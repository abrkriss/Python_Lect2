...
class Matrix:
    def __init__(self, matrix: List[List]):
        ...
        self.__matrix_size = frozenset([(m_rows, len(row)) for row in
                                        self.__matrix])
    ...
    def __add__(self, other: "Matrix") -> "Matrix":
        if not isinstance(other, Matrix):
            raise TypeError(f"'{other.__class__.__name__}' "f"incompatible "
                            f"object type")
...
if __name__ == '__main__':
    matrix1 = Matrix([[20, 50], [54, 88],[10, 23]])
    print(matrix1, '\n')
    print('+')
    matrix2 = Matrix([[10, 20], [30, 85], [55, 10]])
    print(matrix2, '\n')
    print('=')
